...
public class EntityFallingBlockExt extends Entity
{
	...
	protected static final DataParameter<BlockPos> ORIGIN = EntityDataManager.<BlockPos>createKey(EntityFallingBlockExt.class, DataSerializers.BLOCK_POS);
	private static final DataParameter<Optional<IBlockState>> TILE = EntityDataManager.<Optional<IBlockState>>createKey(EntityFallingBlockExt.class, DataSerializers.OPTIONAL_BLOCK_STATE);

	public EntityFallingBlockExt(World worldIn)
	{
		super(worldIn);
	}

	public EntityFallingBlockExt(World worldIn, double x, double y, double z, IBlockState fallingBlockState)
	{
		super(worldIn);
		...
		dataManager.set(TILE, Optional.of(fallingBlockState));
	}
	...
	protected void entityInit()
	{
		...
		this.dataManager.register(TILE, Optional.of(DeliveryBlocks.CRATE.getDefaultState()));
	}
	...
	{
		if(dataManager.get(TILE).isPresent())
		{

			fallTile = dataManager.get(TILE).get();
			Block block = dataManager.get(TILE).get().getBlock();
						...
						if(iblockstate.getBlock() != Blocks.PISTON_EXTENSION)
						{
							...
							if(!this.dontSetBlock)
							{
								if(this.world.mayPlace(block, blockpos1, true, EnumFacing.UP, (Entity) null) && (flag1 || !BlockFalling.canFallThrough(this.world.getBlockState(blockpos1.down()))) && this.world.setBlockState(blockpos1, this.fallTile, 3))
								{
									...
									if(!this.world.isRemote)
									{
										double ii = Math.min((double) (0.2F + 55 / 15.0F), 2.5D);
										int i = (int) (150.0D * ii);
										((WorldServer) this.world).spawnParticle(EnumParticleTypes.BLOCK_DUST, this.posX, this.posY, this.posZ, i, 0.0D, 0.0D, 0.0D, 0.15000000596046448D, Block.getStateId(dataManager.get(TILE).get()));

										SoundEvent event = ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation(DeliveryConfig.landSound));
										world.playSound(null, getPosition(), event, SoundCategory.BLOCKS, 3F, 1F);
									}
